...
class AudioDataset(Dataset):
    def __init__(
        ...
        for name in tqdm(self.paths, total=len(self.paths)):
            ...
            path_ampl = os.path.join(self.path_root, 'ampl', name) + '.npy'
            ampl = np.load(path_ampl)
            ampl = torch.from_numpy(ampl).float()

            path_phase = os.path.join(self.path_root, 'phase', name) + '.npy'
    ...
    def get_data(self, name, data_buffer):
        ...
        # load ampl
        ampl = data_buffer.get('ampl')
        ampl_frames = ampl[:,start_frame : start_frame + mel_frame_len]
        phase = data_buffer.get('phase')
        phase_frames = phase[:,start_frame : start_frame + mel_frame_len]
        ...
        if self.volume_aug:
            max_amp = float(torch.max(torch.abs(audio))) + 1e-5
            max_shift = min(1, np.log(1/max_amp))
            log_mel_shift = random.uniform(-1, max_shift)
            audio *= (np.e ** log_mel_shift)
            audio_mel += log_mel_shift
        #audio_mel = torch.clamp(audio_mel, min=-11.512938235)
